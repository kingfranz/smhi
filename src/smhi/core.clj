...
(defn get-temp-scaling
	[data top bottom]
	(let [min-temp       (if fixed-temp min-fixed-temp (apply min (map second data)))
		  max-temp       (if fixed-temp max-fixed-temp (apply max (map second data)))
  ...
  (let [left-x       (dec left-axis-width)
	...
	(doseq [day-line (range 1 graph-days)]
	  (draw g2d
		...
		day-tick-style))
	; ticks on axises
	(doseq [tick-idx (range axis-span)]
	  ; temp axis
	  (draw g2d
		(line left-x
